<template lang="html">
  <section class="link field" :class="{ dark, localised: showLocalisedOptions }">
    <template v-if="!showLocalisedOptions">
      ...
      <InternalLinkHelper v-if="linkType === 'internal'" :class="{ dark, error, 'in-split': inSplit }" :collections-path="collectionsPath" :dark="dark" :model-value="safeModelValue" :url-suffix="options.urlSuffix" :url-template="template" :use-file-path="!template" @update:model-value="handleInput" />
