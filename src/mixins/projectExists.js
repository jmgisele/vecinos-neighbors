...
export default {
  methods: {
    async projectExists(id, url, userProjects = []) {
      try {
        ...
        if (origin && (origin.url === url || origin.url === url.replace('http', 'https') || origin.url === url.replace('https', 'http'))) return { remote: true, user: userProjects.includes(id) };
        return true;
      } catch (err) {
